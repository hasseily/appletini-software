; Appletini Bosconian -- loads BOSCO.SPR into the auxiliary language card.
;
...
; build/BOSCO.SPR, a file that sits beside BOSCO.SYSTEM on the disk. crt0
...
; directory when a launcher or the boot started BOSCO.SYSTEM); if that
...
; the game overwrites that memory (SHR screen and sprites).
;
...
; BOSCO.SPR itself) the loader does nothing.
...
IOBUF    = $BB00        ; 1 KB ProDOS file buffer: the C stack, unused before main
STAGE_SIZE = 1024
MAX_REGIONS = 8
...
LD_SRC   = ZP_SPARE     ; 2  copy source (main memory)
LD_DST   = ZP_SPARE+2   ; 2  copy destination (auxiliary card)
...
pathname:
        .byte   9, "BOSCO.SPR"
msg_error:
        .byte   "CANNOT LOAD BOSCO.SPR, PRODOS ERROR $00", 0
msg_key:
...
; ---------------------------------------------------------------------------
; build_path2: path2 = directory part of the pathname at $0280 + "BOSCO.SPR".
...
; ---------------------------------------------------------------------------
; copy_piece: stage[0..piece) -> dest in the auxiliary card. ALTZP is on
; while copying, so the zero-page pointers are set after the switch (they
; land in the auxiliary zero page) and the return address, pushed on the
; main stack, is used after switching back.
; ---------------------------------------------------------------------------
copy_piece:
        sta     ALTZPON
        lda     bank
...
; auxiliary memory belongs to the game from here on.
; ---------------------------------------------------------------------------
disconnect_ram:
        lda     MACHID
        and     #$30

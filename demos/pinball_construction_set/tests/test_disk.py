#!/usr/bin/env python3
"""Tests for tools/build_disk.py (the Pinball Construction Set disk).

Builds an image from dummy payloads into a temporary directory and checks
the ProDOS structure with the small reader below, which is written
...
read back. Also: the tables of a directory become BIN files with aux 0,
the writer's storage types, bad names, the command line, and that the
fake ProDOS of tools/a2sim.py serves the very directory blocks this
builder writes for the same files.
...
def walk_directory(image, first=2):
...
def check_volume(tc, image, expected_files, total_blocks):
    ...
    tc.assertEqual(header["name"], VOLUME)
...
def master_prodos():
    ...
    return read_contents(master, next(e for e in master_entries if e["name"] == "PRODOS"))
...
class TestBuildDisk(unittest.TestCase):
    ...
    def test_build_from_dummy_sys(self):
        ...
        prodos = master_prodos()
        self.assertGreater(len(prodos), 8192)
        check_volume(self, image, {
            "PRODOS": (0xFF, 0x0000, prodos),
            "PCS.SYSTEM": (0xFF, 0x2000, payload),
        }, 1600)

    @unittest.skipUnless(MASTER.is_file(), f"ProDOS master not found: {MASTER}")
    def test_build_with_sprites_and_tables(self):
        payload = bytes([0x78, 0xD8]) + os.urandom(3000)
        sprites = b"BSPR" + bytes([1, 0x00, 0xD0, 0x00, 0x10, 0x04]) + os.urandom(0x1000)
        tables = {"TABLE1.PCS": b"PCS1" + os.urandom(700) + b"OVL1" + bytes(72) + b"\0",
                  "MINE.PCS": b"PCS1" + os.urandom(1500) + b"OVL1" + bytes(72) + b"\0"}
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "test.hdv"
            build_disk.build(payload, MASTER, out, sprites=sprites, tables=tables)
            image = out.read_bytes()
        expected = {
            "PRODOS": (0xFF, 0x0000, master_prodos()),
            "PCS.SYSTEM": (0xFF, 0x2000, payload),
            "PCS.SPR": (0x06, 0xD000, sprites),
        }
        for name, data in tables.items():
            expected[name] = (0x06, 0x0000, data)
        check_volume(self, image, expected, 1600)
        # the program's catalog reader wants the BIN files named *.PCS
        _, entries, _ = walk_directory(image)
        catalog = [e["name"] for e in entries if e["type"] == 6 and e["name"].endswith(".PCS")]
        self.assertEqual(catalog, ["TABLE1.PCS", "MINE.PCS"])

    def test_read_tables(self):
        with tempfile.TemporaryDirectory() as tmp:
            d = Path(tmp)
            (d / "B.PCS").write_bytes(b"b")
            (d / "a.pcs").write_bytes(b"a")
            (d / "notes.txt").write_bytes(b"n")
            self.assertEqual(build_disk.read_tables(d), {"A.PCS": b"a", "B.PCS": b"b"})
    ...
    def test_cli(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            tables.mkdir()
            (tables / "T.PCS").write_bytes(b"PCS1" + bytes(40))
            out = Path(tmp) / "cli.hdv"
            r = subprocess.run([sys.executable, str(ROOT / "tools/build_disk.py"),
                                "--system", str(sys_path), "--tables", str(tables),
                                "--output", str(out)],
                               capture_output=True, text=True)
            self.assertEqual(r.returncode, 0, r.stdout + r.stderr)
            self.assertIn("PCS.SYSTEM", r.stdout)
            self.assertIn("T.PCS", r.stdout)
            self.assertIn("volume A13PCS", r.stdout)
    ...
    def test_fake_prodos_serves_the_same_directory(self):
        """a2sim's FakeProDOS reads the volume directory as the blocks this
        writer produces for the same files (so the program's catalog
        parser sees one layout in the tests and on the disk)."""
        import a2sim
        files = {"PCS.SYSTEM": (0xFF, 0x2000, bytes(3000)), "PCS.SPR": (0x06, 0xD000, bytes(700)),
                 "TABLE1.PCS": bytes(785), "MINE.PCS": bytes(1200)}
        fake = a2sim.FakeProDOS(files)
        blocks = fake.directory_blocks()
        writer = build_disk.VolumeWriter()
        for name, value in files.items():
            ftype, aux, data = value if isinstance(value, tuple) else (0x06, 0, value)
            writer.add_file(name, data, ftype, aux)
        image = writer.finish()
        self.assertEqual(blocks, image[2 * BLOCK:6 * BLOCK])
        # walk the served blocks as a 6-block image whose directory starts at block 2
        padded = bytes(2 * BLOCK) + blocks
        header, entries, chain = walk_directory(padded)
        self.assertEqual(header["name"], VOLUME)
        self.assertEqual([e["name"] for e in entries], list(files))
        self.assertEqual([e["type"] for e in entries], [0xFF, 0x06, 0x06, 0x06])

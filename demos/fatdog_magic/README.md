...
switch sequences follow the
[Appletini demo viewer](../appletini_demos/a2imgview.a65).
...
generated by appletini-one/scripts/gen_hgr_assets.py, now maintained as the
[Appletini demo asset generator](../appletini_demos/tools/gen_hgr_assets.py).
The builder generates the FATDOG title and HGR row tables from this bundled font.
...
```sh
python demos/fatdog_magic/tools/smoke_test.py --gssquared-root PATH_TO_GSSQUARED --formats-from .
```

Run this from the appletini-software repository root; --formats-from also
accepts the Appletini demo project or its assets directory, and a legacy
appletini-one checkout. This creates a separate 800K validation disk, mixes
all formats in one folder, verifies every destination byte, tests help/reload for every format,
